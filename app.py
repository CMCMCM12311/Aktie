...
# Apple Design System - UI Layout Update
st.markdown("""
    <style>
        ...
        .section-label { font-size: 0.85rem; color: #86868b; margin: 15px 0 5px 0; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;}
    </style>
...
# --- DATA FUNKTION (NU MED PRIS I USD) ---
@st.cache_data(show_spinner=False)
def get_price_data(tickers_str, years_val):
    try:
...
data = get_price_data(st.session_state.tickers_val, st.session_state.years_val)
...
if data is not None:
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            ...
            yaxis=dict(title="Pris i USD", showgrid=True, gridcolor='#f5f5f7', fixedrange=True),
    ...
    st.info("Brug knapperne nedenfor eller søg for at se data.")

# --- KONTROLPANEL (RYKKET RUNDT) ---
st.write("---")

# 1. Tidshorisont (Nu øverst i panelet)
...
# 2. Søg / Tickers
st.markdown("<div class='section-label'>2. Tilføj selskaber eller krypto</div>", unsafe_allow_html=True)
search_input = st.text_input("Søg", value=st.session_state.tickers_val, placeholder="AAPL, BTC-USD, NVDA...", label_visibility="collapsed")

if search_input != st.session_state.tickers_val:
    st.session_state.tickers_val = search_input
    st.rerun()

# 3. Populære (Indsætter nu i feltet)
st.markdown("<div class='section-label'>Hurtig tilføj (Populære)</div>", unsafe_allow_html=True)
row1 = st.columns(10)
row2 = st.columns(10)
...
for i, ticker in enumerate(popular_tickers):
    ...
    if col.button(ticker, key=f"pop_{ticker}"):
        # Hent nuværende tickers, tilføj den nye hvis den ikke er der
        current_list = [t.strip().upper() for t in st.session_state.tickers_val.split(",") if t.strip()]
        if ticker not in current_list:
            current_list.append(ticker)
            st.session_state.tickers_val = ", ".join(current_list)
            st.rerun()
